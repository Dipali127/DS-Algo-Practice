...
// I will use two stacks to store the previous smaller index and the next smaller index for the current bar. 
...
//
// So, I will call a function `prevSmaller`, which returns the indices of all the previous smaller heights 
// for each bar. Then, I will call a function `nextSmaller`, which returns the indices of all the next smaller 
// heights for each bar. 
// I will then iterate through the `height` array, and for each bar, compute the width using the 
...
// While the stack is not empty and the height at the index on top of the stack is greater than or equal 
...
// than the height at the index on top of the stack. Update the `ns` array at the popped index with the current index.
