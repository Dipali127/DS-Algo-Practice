...
// If the target is found (i.e., nums[i] >= target), return the index 'i' as the target should be inserted
// at or before it.
...
var searchInsert = function (nums, target) {
  ...
  // calculate the middle index, mid, using the formula: mid = Math.floor(low + (high - low) / 2).
  // This avoids potential overflow that could occur when using (low + high) / 2 in some languages.
  // if nums[mid] is equal to target, return mid because the target is found.
  // else if nums[mid] is less than target, set low = mid + 1 to search the right half of the array 
  // (since target must be in the higher half).
  // else, set high = mid - 1 to search the left half of the array (since the target must be in the 
  // lower part).
  // continue the loop until low exceeds high.
  // if the loop ends without finding the target, return low because it represents the position where
  // the target should be inserted to maintain the sorted order.
