...
// Approach:
// Consider each possible subarray and for each subarray, compute the sum and check if the sum equals the
// target.
...
// Meanwhile, check if the `sum` equals the target. If it does, return the current window indices 
// [start + 1, end + 1].
// If the `sum` becomes greater than the target, shrink the window by subtracting the number at the
//  `start` pointer
