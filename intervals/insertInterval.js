...
// and check if the start time of the current interval is greater than the end time of newInterval, it means I have found the correct position to insert newInterval. I will insert it using splice() and return immediately, as all overlapping intervals are already merged.
// otherwise, if the end time of the current interval is greater than or equal to the start time of newInterval, it means they overlap. I will merge them by updating newInterval to have the minimum start time and maximum end time of both intervals. Then, I will remove the merged interval using splice().
...
// TC:- O(N^2), because in the worst case (where all intervals overlap with newInterval), the time complexity could be O(N²) due to the repeated calls to splice() and the shifting of elements.
...
var insert = function (intervals, newInterval) {
    ...
    while (i < intervals.length) {
        if (intervals[i][0] > newInterval[1]) {
            ...
            intervals.splice(i, 1); // Remove the merged interval
        } else {
            i++;
        }
    }

    // If newInterval wasn't inserted inside the loop, add it at the end
...
// Optimal Approach:
// approach:
// instead of modifying the existing intervals array (which could unnecessarily increase time complexity), I will use a result array to store the newInterval and all non-overlapping intervals.
// i will run a while loop to iterate through the intervals array. During each iteration, I will check that:
// If the current interval's end time is less than the newInterval's start time, this means the current interval is non-overlapping. I will insert it into the result array.
