...
// Approach:-
// call the function 'common' with arguments string1, string2, index 'i', index 'j', and lengths:- length1, length2.
// Inside the 'common' function:
//   - Check if index 'i' (pointing to string1) or index 'j' (pointing to string2) is out of bounds. 
//      if either index is out of bounds, return 0 as there are no more characters to compare.
//   - If the characters at the current indices (string1[i] and string2[j]) are equal:
//             - Add 1 to the result and recursively call the function with the next indices (i + 1, j + 1) 
//                 to continue checking for the next characters.
...
// TC:- O(2^M * 2^N) = O(2^M+N), where 'M' is the length of string1 and 'N' is the length of string2.
// Explanation:- for each index 'i', there are two recursive calls: one skipping a character from string1 and one skipping a 
// character from string2. The problem explores all combinations of subsequences, resulting in an exponential time complexity.
// SC:- O(min(N1, N2)), due to the recursion stack as the maximum depth of the recursion is determined 
...
var longestCommonSubsequence = function(text1, text2) {
    let length1 = text1.length, length2 = text2.length;
    let dp = Array.from(Array(length1+1), () => Array(length2+1).fill(-1));
...
// for the substrings text1[0..i-1] and text2[0..j-1].
// In dp[i][j], 'i' and 'j' are lengths of the substrings text1 and text2 considered up to indices from 0 to i-1 and
// 0 to j-1, respectively, not the actual indices of the characters.
...
//    - If the characters are not equal, set dp[i][j] = max(dp[i-1][j], dp[i][j-1]), as we either skip a character from text1 or text2.
...
var longestCommonSubsequence = function(text1, text2) {
    ...
    for(let row = 0; row < length1+1; row++){
        dp[row][0] = 0;
    }

    for(let col = 0; col < length2+1; col++){
        dp[0][col] = 0;
    }
